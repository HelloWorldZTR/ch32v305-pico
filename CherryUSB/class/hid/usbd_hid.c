    ...
    if (((setup->bmRequestType & USB_REQUEST_DIR_MASK) == USB_REQUEST_DIR_IN) &&
        ...
        switch (value) {
            case HID_DESCRIPTOR_TYPE_HID:
                ...
                if (current_hid_class->hid_descriptor == NULL) {
                    return -2;
                }
            ...
            case HID_DESCRIPTOR_TYPE_HID_REPORT:
                ...
                if (current_hid_class->hid_report_descriptor == NULL) {
                    return -2;
                }
...
static int hid_class_request_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    ...
    switch (setup->bRequest) {
        case HID_REQUEST_GET_REPORT:
        {
            ...
            /* Never offer more data than the host requested on EP0. */
            if (report_len > setup->wLength) {
                report_len = setup->wLength;
            }
            *data = (uint8_t *)report;
            *len = report_len;
            break;
