...
/**
 * @brief Locate a normal-speed configuration descriptor by value.
 *
 * @param configuration Configuration value selected by the host.
 * @return Descriptor pointer when present, otherwise NULL.
 */
static const struct usb_configuration_descriptor *
usbd_find_configuration(uint8_t configuration)
{
    ...
    while((p != NULL) && (p[DESC_bLength] != 0U))
    {
        if((p[DESC_bDescriptorType] == USB_DESCRIPTOR_TYPE_CONFIGURATION) &&
           (p[CONF_DESC_bConfigurationValue] == configuration))
        {
            return (const struct usb_configuration_descriptor *)p;
        }
        p += p[DESC_bLength];
    }
    return NULL;
    ...
    while((p != NULL) && (p[DESC_bLength] != 0U))
    {
        ...
        else if(p[DESC_bDescriptorType] == USB_DESCRIPTOR_TYPE_OTHER_SPEED)
        {
            in_active_config = false;
                ...
                (p[INTF_DESC_bInterfaceNumber] == interface))
        {
            return true;
        }
        p += p[DESC_bLength];
...
static bool is_ep_valid(uint8_t ep)
{
    ...
    if ((ep & 0x7f) == 0) {
        return (ep == USB_CONTROL_IN_EP0) || (ep == USB_CONTROL_OUT_EP0);
    }
    ...
    while (p[DESC_bLength] != 0U) {
        switch (p[DESC_bDescriptorType]) {
            case USB_DESCRIPTOR_TYPE_CONFIGURATION:
                ...
                in_active_config = (cur_config == config_index);
            ...
            case USB_DESCRIPTOR_TYPE_ENDPOINT:
                if (!in_active_config || (cur_config != config_index) ||
                    (cur_alt_setting != alt_setting)) {
                    break;
                }

                if(!usbd_set_endpoint((struct usb_endpoint_descriptor *)p))
                {
                    success = false;
                }
                break;
...
/**
 * @brief Return the active configuration attributes byte.
 *
 * @return bmAttributes for the active configuration, or bus-powered default.
 */
static uint8_t usbd_active_configuration_attributes(void)
{
    const struct usb_configuration_descriptor *configuration;

    configuration = usbd_find_configuration(usbd_core_cfg.configuration);
...
static bool usbd_std_device_req_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    ...
    switch (setup->bRequest) {
        case USB_REQUEST_GET_STATUS:
            ...
            /* bit 0: self-powered */
            /* bit 1: remote wakeup */
            attributes = usbd_active_configuration_attributes();
            usbd_core_cfg.status_response = 0U;
            if((attributes & USB_CONFIG_SELF_POWERED) ==
               USB_CONFIG_SELF_POWERED)
            {
                usbd_core_cfg.status_response |= USB_GETSTATUS_SELF_POWERED;
            }
            if(usbd_core_cfg.remote_wakeup != 0U)
            {
                usbd_core_cfg.status_response |= USB_GETSTATUS_REMOTE_WAKEUP;
            }
            *data = (uint8_t *)&usbd_core_cfg.status_response;
            *len = 2;
            break;
        ...
        case USB_REQUEST_CLEAR_FEATURE:
            ...
            if((value == USB_FEATURE_REMOTE_WAKEUP) &&
               ...
               ((attributes & USB_CONFIG_REMOTE_WAKEUP) != 0U)) {
        ...
        case USB_REQUEST_SET_CONFIGURATION:
            ...
            if(value == usbd_core_cfg.configuration)
            {
                break;
            }
            if((value != 0U) &&
               (usbd_find_configuration((uint8_t)value) == NULL))
            {
                ret = false;
                break;
            }

            previous_configuration = usbd_core_cfg.configuration;
            close_ok = true;
                                           ...
                                           uint8_t **data, uint32_t *len)
{
     ...
     */
    if ((setup->wIndex & 0xff00U) != 0U || !is_device_configured() ||
    ...
    switch (setup->bRequest) {
        ...
        case USB_REQUEST_SET_INTERFACE:
            ...
            if(!usbd_set_interface((uint8_t)setup->wIndex,
...
static bool usbd_std_endpoint_req_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    ...
    if (((setup->wIndex & 0xff00U) != 0U) || !is_ep_valid(ep)) {
        return false;
    }
    ...
    switch (setup->bRequest) {
        case USB_REQUEST_GET_STATUS:
            ...
            if (((ep & 0x7f) == 0) || is_device_configured()) {
                ...
                uint8_t stalled = 0U;
                if(usbd_ep_is_stalled(ep, &stalled) != 0)
                {
                    return false;
                }
                usbd_core_cfg.status_response = stalled ? 1U : 0U;
                *data = (uint8_t *)&usbd_core_cfg.status_response;
        ...
        case USB_REQUEST_CLEAR_FEATURE:
            if (setup->wValue == USB_FEATURE_ENDPOINT_HALT) {
                ...
                if (((ep & 0x7f) != 0) && is_device_configured()) {
                    ...
                    if(usbd_ep_clear_stall(ep) != 0)
                    {
                        return false;
                    }
                    usbd_event_notify_handler(
                        USBD_EVENT_CLEAR_HALT, (void *)(unsigned long)ep);
                    break;
                }
            }
        ...
        case USB_REQUEST_SET_FEATURE:
            if (setup->wValue == USB_FEATURE_ENDPOINT_HALT) {
                ...
                if (((ep & 0x7f) != 0) && is_device_configured()) {
                    ...
                    if(usbd_ep_set_stall(ep) != 0)
                    {
                        return false;
                    }
                    usbd_event_notify_handler(
                        USBD_EVENT_SET_HALT, (void *)(unsigned long)ep);
                    break;
                }
            }
...
/**
 * @brief Detach the controller and clear the active configuration state.
 */
int usbd_deinitialize(void)
{
    uint8_t previous_configuration = usbd_core_cfg.configuration;
